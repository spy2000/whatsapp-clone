...
function ChatContainer() {
  ...
  return <div className="h-[80vh] w-full relative flex-grow overflow-auto custom-scrollbar">
    ...
    <div className="bg-chat-background bg-fixed h-full w-full absolute left-0 top-0">
        ...
        <div className="flex flex-col justify-end w-full gap-1 overflow-auto">
          {messages && messages.map((message,index)=>(
            <div key={message.id} className={`flex ${message.senderId === currentChatUser.id ? "justify-start" : "justify-end"}`}>
              {message.type === "text" && (<div className={`text-white px-2 py-[5px] tex-sm rounded-sm flex gap-2 items-end max-w-[45%] ${message.senderId === currentChatUser.id ? "bg-incoming-background" : "bg-outgoing-background"}`}>
